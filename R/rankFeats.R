...
rankFeats <- function(my.relab, my.effsiz, my.qvalue, mod.assn, category){
  # packages
  library(psych)
  ...
  all.param$status <- ""
  all.param[which(all.param$efs < 0),"status"] <- "depleted"
  all.param[which(all.param$efs > 0),"status"] <- "enriched"
  ...
  all.param$meta_rank <- harmonic.mean(t(all.param[,7:9]))
